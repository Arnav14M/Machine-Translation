...
class Translator(nn.Module):
    def __init__(self):
        ...
        self.lstm_dec = nn.LSTM(self.embed_size+self.H,self.H,num_layers=1,dropout=self.drop)
        self.out = nn.Linear(self.H,vocab_len)
        self.att = nn.Linear(2*self.H,1)
        self.relu = nn.ReLU()
    ...
    def forward(self,src,tgt):
        ...
        h_states , (hidden,cell) = self.lstm_enc(embedded_ip,(h,c))
        ...
        for i in range(1,tgt.shape[0]):
            ...
            #attention
            hidden_rep = hidden.repeat(h_states.shape[0], 1, 1)
            joined = torch.cat((hidden_rep, h_states), dim=2)
            attention = self.softmax(self.relu(self.att(joined)))

            context = torch.sum(attention*h_states,dim=0).unsqueeze(0)

            fresh_ip = torch.cat((context,embedded_op),dim=2)
            
            outputs , (hidden,cell) = self.lstm_dec(fresh_ip,(hidden,cell))
...
if TRAIN:
    ...
    torch.save(model, "./Model/new_lstm_att.pth")
else:
    model = torch.load("./Model/new_lstm_att.pth")
