...
class encoder(nn.Module):
    def __init__(self):
        ...
        self.attention = nn.Linear(L2,1)
        self.H=L2
        

    def forward(self, x):
        weighted_h = torch.zeros((1,self.H)).to(device)
        ...
        for i in range(x.shape[0]):
            ...
            tot+=att[0][0]
            weighted_h+=att[0][0]*h
        return (weighted_h/tot,c)
...
if TRAIN:
    ...
    torch.save(enc, "./Model/LSTM_att_enc1.pth")
    torch.save(dec, "./Model/LSTM_att_dec1.pth")

else:
    enc = torch.load("./Model/LSTM_att_enc1.pth")
    dec = torch.load("./Model/LSTM_att_dec1.pth")
